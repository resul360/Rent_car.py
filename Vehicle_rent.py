...
class VehicleRent:
    ...
    def __init__(self,stock):  # araç stok kontrolü
        self.stock=stock
        self.now=0    #initial bill value

    def dispalyStock(self):
        """
        display stock
        """
    ...
    def rentHourly(self, n):
        """
        rent Hourly    # n : number of rent hourly car
        """
        if n<=0:
    ...
    def returnVehicle(self, request, brand):
        ...
        if brand=="car":
            if rentalTime and rentalBasis and numOfVehicle:
                self.stock += numOfVehicle  # my garage is uptaded
...
class CarRent(VehicleRent):
    ...
    global discound_rate
    discound_rate=15
    
    def __init__(self, stock):
        super().__init__(stock)  # if you do initilazing, this function will use all variable and fucvtion from parent
